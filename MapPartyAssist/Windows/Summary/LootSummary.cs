...
namespace MapPartyAssist.Windows.Summary {
    internal class LootSummary {
        ...
        public void Refresh(List<DataFilter> filters) {
                        ...
                        case Type _ when filter.GetType() == typeof(TimeFilter):
                            ...
                            switch(timeFilter.StatRange) {
                                ...
                                case StatRange.SinceLastClear:
                                    var lastClear = _plugin.StorageManager.GetDutyResults().Query().Where(dr => dr.IsComplete).OrderBy(dr => dr.Time).ToList().Where(dr => dr.CheckpointResults.Count == _plugin.DutyManager.Duties[dr.DutyId].Checkpoints!.Count && dr.CheckpointResults.Last().IsReached).LastOrDefault();
                                    if(lastClear != null) {
                                        dutyResults = _plugin.StorageManager.GetDutyResults().Query().Where(dr => dr.Time > lastClear.Time).ToList();
...
//            //should copy data to prevent race conditions if it changes during filtering
